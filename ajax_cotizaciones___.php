                ...
                <div>
                  <table>
                    <tbody class="cotizacion_pagos_lista">
                      <tr><th class="gray">Pago</th><th class="gray">Porcentaje</th><th class="gray">Forma</th><th class="gray">Monto</th><th class="gray"></th></tr>
      ...
      if($result_registros = mysqli_query($conexion, $sql_registros)){
          if(mysqli_num_rows($result_registros) > 0){
              ...
              while ($row_registros = mysqli_fetch_array($result_registros)){
                ...
                $item = utf8_encode($row_registros['item']);
                $categoria = utf8_encode($row_registros['categoria_cotizacion']);

                $sql_promedio = "SELECT SUM(importe_total) AS suma FROM registros WHERE item = '$item' AND registro_seleccionado = 1 AND categoria_cotizacion = '$categoria'";
                ...
                if($datos_promedio["suma"] == 0){
                  $sql_promedio0 = "SELECT AVG(importe_total) AS suma FROM registros WHERE item = '$item' AND registro_seleccionado = 0 AND categoria_cotizacion = '$categoria'";
...
>>>>>>> 00059c6bec1b069e4d59dcacc16293f16d9945c3
          ...
          if ($row_registros['mensaje_cotizacion'] == 0){
            ...
            echo "<table border='0' cellspacing='0' cellpadding='0' width='100%' style='border: 3px solid #fff !important;'>";
              echo "<tbody>";
                echo "<tr>";
                  echo "<td class='seleccion_item' data-rubro='".$row_registros['id_rubros_cotizaciones']."' data-categoria='".$row_registros['id_catcot']."' data-item='".$row_registros['item']."'>".$row_registros['item']."</td>";
                echo "</tr>";
                echo "<tr>";
                  echo "<td>".$texto." $<span class='numerable'>".$datos_promedio['suma']."</td>";
                echo "</tr>";
              echo "</tbody>";
            echo "</table>";
          echo "</td>";
