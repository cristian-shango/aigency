      ...
      if(mysqli_num_rows($result) > 0){
        while ($row = mysqli_fetch_array($result)) {
          echo "<tr class='cotizacion_pagos_group'>";
            ...
            echo "<td>$<span class='valor_promedio numerable cotizacion_pagos_total' data-registro='".$row['id']."' data-valor='".$row['importe_total']."'>".$row['importe_total']."</span></td>";
            echo "<td><button type='button' class='btn btn-success procesar_modificacion' data-registro='".$row['id']."' data-valor='".$row['importe_total']."' data-estado='".$estado."' data-opcion='0'><i class='icon wb-check' aria-hidden='true'></i></button><button type='button' class='btn btn-danger procesar_modificacion' data-registro='".$row['id']."' data-valor='".$row['importe_total']."' data-estado='".$estado."' data-opcion='1'><i class='icon wb-close' aria-hidden='true'></i></button></td>";
          echo "</tr>";
        }
        ...
        if(mysqli_num_rows($result_eliminados) > 0){
          while ($row_eliminados = mysqli_fetch_array($result_eliminados)) {
            ...
            echo "<td>$<span class='valor_promedio numerable cotizacion_pagos_total' data-registro='".$row_eliminados['id']."' data-valor='".$row_eliminados['importe_total']."'>".$row_eliminados['importe_total']."</span></td>";
            echo "<td><button type='button' class='btn btn-success procesar_modificacion' data-registro='".$row_eliminados['id']."' data-valor='".$row_eliminados['importe_total']."' data-estado='".$row_eliminados['estado_registro']."' data-opcion='0'><i class='icon wb-check' aria-hidden='true'></i></button><button type='button' class='btn btn-danger procesar_modificacion' data-registro='".$row_eliminados['id']."' data-valor='".$row_eliminados['importe_total']."' data-estado='".$row_eliminados['estado_registro']."' data-opcion='1'><i class='icon wb-close' aria-hidden='true'></i></button></td>";
            echo "</tr>";
          }
        }
      }
