<?php  
 ...
 //fetch.php  
	$sql = "SELECT * FROM facturas_cotizaciones fc LEFT JOIN registros r ON fc.id_registro_cotizacion = r.id  LEFT JOIN proveedores p ON r.id_proveedor = p.id_proveedor LEFT JOIN proyectos pro ON r.id_proyecto = pro.id LEFT JOIN clientes c ON pro.cliente = c.id_cliente LEFT JOIN forma_pago fp ON fc.id_forma_pago_cotizacion = fp.id LEFT JOIN tipo_factura tf ON fc.id_tipo_factura_cotizacion = tf.id_factura LEFT JOIN item_cotizacion it ON r.item = it.id_item_cotizacion WHERE r.registro_seleccionado = 1 AND fc.id_factura_cotizacion = '$id'";
