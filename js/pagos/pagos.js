...
var cotizacion_pagos = (function(){
  ...
  function bind_eventos(html=document){
    ...
    $(html).find(".cotizacion_pagos_cargar_factura").click(function(){
      factura_esto($(this).closest(".cotizacion_pagos_data")[0]);
    });
  ...
  function escribir_data(data, valor) {
    //console.log("escribiendo ", valor, " en ", tag);
    if ("value" in tag) tag.value = valor;
    else ($(tag).text(valor));
  }
  ...
  function leer_datos(html){
    ...
    pago_datos.monto = leer_tag($(html).find(".cotizacion_pagos_monto")[0]);
  ...
  function cargar_pago_registro(registro) {
    ...
    Object.keys(pagos[registro]).forEach(function(pago){
      ...
      $data = $data_modelo.clone().attr("data-id_pago", pago).appendTo($lista).each(function(ix, data){
          ...
          $(this).closest("tr").find(".cotizacion_pagos_cargar_factura").attr("data-id_pago", pago);
          let proveedor = $("#edicion_proveedor").val();
          $(this).closest("tr").find(".cotizacion_pagos_cargar_factura").attr("data-id_proveedor", proveedor);
      });
  ...
  function borrar_esto(html) {
      ...
      let id_pago = pago.id_pago;
      var proyecto = document.getElementById('ingreso_id').innerHTML;
      let id_proy_reg_pago = proyecto+reg+id_pago;
      if (pagos[reg].hasOwnProperty(pago.id_pago)) {
          $(datohtml).remove();
          delete pagos[reg][pago.id_pago];
          $.ajax({
            url:"ajax/eliminar_factura_proveedor.php",
            method:"POST",
            data: 'id_proy_reg_pago='+id_proy_reg_pago,
            dataType:"json"
          });
  ...
  function factura_esto(html) {
      let $listahtml = $(html).closest(".cotizacion_pagos_lista");
      let datohtml = $(html).closest(".cotizacion_pagos_data")[0];
      let pago = leer_datos(datohtml);
      let registro = pago.registro;
      console.log("Registro a cargar Factura: ", registro);
      let id_pago = pago.id_pago;
      let plazo = pago.plazo;
      let porcentaje = pago.porcentaje;
      let forma = pago.forma;
      let fecha1 = pago.fecha1;
      let fecha2 = pago.fecha2;
      let monto = pago.monto;
      var proyecto = document.getElementById('ingreso_id').innerHTML;

      console.log(pago.registro);
      console.log(pago.id_pago);
      console.log(pago.plazo);
      console.log(pago.porcentaje);
      console.log(pago.forma);
      console.log(pago.fecha1);
      console.log(pago.fecha2);
      console.log(pago.monto);

      let id_proy_reg_pago = proyecto+registro+id_pago;
      console.log("ID Cotizacion + Registro + Pago: ",id_proy_reg_pago);

      $.ajax({
        url:"ajax/comprobar_factura_proveedor.php",
        method:"POST",
        data: 'id_proy_reg_pago='+id_proy_reg_pago,
        dataType:"json",
        success:function(datos){
          ...
          $("#importe_neto").val('');
          $("#forma_pago_nuevo").val('');
          $("#tiempo_pago").val('');
          $("#fecha_facturacion").val('');
          $("#numero_factura_nuevo_registro").val('');
          $("#fecha_pago").val('');
          $("#tipo_factura_nuevo_registro").val('');
          $("#tipo_iva").val('');
          $("#iva").val('');
          $("#percepcion").val('');
          $("#importe_bruto").val('');
          //
          if (datos != null){
            console.log("ID Factura traido de la DB: ",datos.id_factura_cotizacion);
            if (datos.id_factura_cotizacion == id_proy_reg_pago){
              console.log("Factura existente");
              if (datos.id_tipo_factura_cotizacion == '1' || datos.id_tipo_factura_cotizacion == '2' || datos.id_tipo_factura_cotizacion == '5') {
              ...
              $("#importe_neto").val(datos.importe_neto_cotizacion);
              $("#forma_pago_nuevo").val(datos.id_forma_pago_cotizacion);
              $("#tiempo_pago").val(datos.tiempo_pago_cotizacion);
              $("#fecha_facturacion").val(datos.fecha_factura_cotizacion);
              $("#numero_factura_nuevo_registro").val(datos.numero_factura_cotizacion);
              $("#fecha_pago").val(datos.fecha_pago_cotizacion);
              $("#tipo_factura_nuevo_registro").val(datos.id_tipo_factura_cotizacion);
              $("#tipo_iva").val(datos.id_tipo_iva_cotizacion);
              ...
              $("#boton_guardar_factura_cotizacion").attr('data-id_registro', registro);
              let archivo_adjunto = datos.factura_adjunta_cotizacion;
              //archivo_adjunto = archivo_adjunto.substring(10);
              console.log(archivo_adjunto);

              //$("#subir_archivo_adjunto").css("display", "none");
              if (archivo_adjunto == null){
                $("#mostrar_archivo_adjunto").css("display", "none");
              } else {
                $("#mostrar_archivo_adjunto").css("display", "block");
                $("#archivo_adjunto").attr("href", "https://admin.vivashango.com/uploads/factura_proveedores/"+id_proy_reg_pago+"_factura_proveedor_"+archivo_adjunto);
              }    
              
              $('#modal_cargar_proveedor').modal('hide');
              $('#modal_factura_cotizacion').modal('show');
            }
          } else {
            $("#importe_neto").val(pago.monto);
            $("#forma_pago_nuevo").val(pago.forma);
            $("#tiempo_pago").val(pago.plazo);
            $("#fecha_facturacion").val(pago.fecha1);
            $("#boton_guardar_factura_cotizacion").attr('data-id_pago', id_proy_reg_pago);
            $("#boton_guardar_factura_cotizacion").attr('data-id_registro', registro);
            ...
            $("#subir_archivo_adjunto").css("display", "block");
            $("#mostrar_archivo_adjunto").css("display", "none");    
            $('#modal_cargar_proveedor').modal('hide');
            $('#modal_factura_cotizacion').modal('show');
          }
