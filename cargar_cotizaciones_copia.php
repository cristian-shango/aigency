					...
					<div class="table-responsive">
									...
									if($result = mysqli_query($conexion, $sql)){
									    ...
									    if($cantidad > 0){
									        ...
									        while($row = mysqli_fetch_array($result)){
												...
												$sql_promedio = "SELECT AVG(importe_total) AS suma FROM registros WHERE categoria_cotizacion = $id_catcot AND registro_seleccionado = 0";
						...
						<table class="table" id="table_format">
								...
								<?php
									...
									$sql = "SELECT * FROM adicionales a RIGHT JOIN registros r ON a.id_cotizacion_adicional = r.id WHERE id_proyecto_adicional = '".$_GET['id']."'";
									mysql_query("SET NAMES 'utf8'");
									if($result = mysqli_query($conexion, $sql)){
								...
								?>
										<tr>
											<td><?php echo ($row['detalle_registro']);?></td>
											<td><?php echo ($row['cantidad']);?></td>
											<td>$ <?php echo ($row['importe_neto']);?></td>
											<td>$ <?php echo ($row['importe_total']);?></td>
											<td><?php echo (utf8_encode($row['motivo_adicional']));?></td>
											<td>$ <?php echo ($row['monto_adicional']);?></td>
											<?php
												if ($row['aprobado_adicional'] == 0){
											?>
													<td><strong><i class="far fa-circle"></i></strong></td>
											<?php
												} elseif ($row['aprobado_adicional'] == 1) {
											?>
													<td><strong><i class="far fa-check-circle"></i></strong></td>
											<?php
												} else {
											?>
												<td><strong><i class="fas fa-ban"></i></strong></td>
											<?php
												}
											?>
										</tr>
