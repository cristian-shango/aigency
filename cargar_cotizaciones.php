...
<html class="no-js css-menubar" lang="en">
  ...
  <body class="animsition page-aside-static page-aside-right">
                  	...
                  	<div id="mostrar_costo_presupuestado" class="cotizacion_pagos_totales_plazo col-md-12">
                    	<div class="form-group">
                        	...
                        	<div class="input-group">
                        		...
                        		<input type="text" class="form-control cotizacion_pagos_totales_monto" readonly>
                      		</div>
                		</div>
                  	</div>
                </div>
                      ...
                      <div class="chat-body">
                        <div class="chat-content">
                          <p>
                            Hola...                          </p>
                          <p>Estado: 3</p>
                          <time class="chat-time" datetime="2019-12-05 17:49:10">2019-12-05 17:49:10</time>
                        </div>
                      </div>
                    </div>-->
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <label for="rubro"><input type="checkbox" id="rubro" value="1">Rubro</label>
                    <label for="rubro"><input type="checkbox" id="rubro" value="1">Rubro</label>
                    <textarea row="10" id="escribir_mensaje" class="form-control" aria-label="Default" aria-describedby="inputGroup-sizing-default" maxlength="140"></textarea>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="col-md-12">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control form-control-lg" id="monto_adicional">
                  </div>
                </div>
              </div>
              <div class="col-md-12">
    ...
    <div class="modal fade" id="modal_factura_cotizacion" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-center" role="document">
        <div class="modal-content">
          <form name="formulario_mensaje">
            <div class="modal-body">
						              ...
						              <?php
            						    ...
            						    if($result1 = mysqli_query($conexion, $sql1)){
            						        if(mysqli_num_rows($result1) > 0){
            						            ...
            						            while($row1 = mysqli_fetch_array($result1)){
            						              ?>
            						                <option value="<?php echo ($row1['id_factura']);?>"><?php echo (utf8_encode($row1['tipo_factura']));?></option>
            						              <?php
            						            }
            						            // Free result set
            						            mysqli_free_result($result1);
          ...
          <div class="col-md-12" id="mostrar_archivo_adjunto">
            <h4><a href="" target="_blank" id="archivo_adjunto"><i class="icon wb-file" aria-hidden="true" style="font-size: 20px;"></i>VER FACTURA ADJUNTA</a></h4>
					...
					<div class="col-md-12" id="subir_archivo_adjunto">
						<input type="file" class="upload_file" data-plugin="dropify" data-default-file=""/>
					</div>
					
					<div class="col-md-6">
						<button type="button" class="btn btn-success btn-block" id="boton_guardar_factura_cotizacion" data-plugin="toastr" data-message="FACTURA GUARDADA CORRECTAMENTE" data-container-id="toast-topFullWidth" data-position-class="toast-top-full-width" data-show-method="slideDown" data-icon-class="toast-just-text toast-success toast-shadow" data-progress-bar="true" data-dismiss="modal" data-toggle="modal" data-target="#modal_cargar_proveedor"><strong>GUARDAR</strong></button>
					</div>
					<div class="col-md-6">
						<button type="button" class="btn btn-danger btn-block" data-dismiss="modal" data-toggle="modal" data-target="#modal_cargar_proveedor"><strong>CANCELAR</strong></button>
					</div>
      	</div>
    	</div>
  	</form>
            ...
            <div class="tab-pane fade show active" id="tab_editar_cotizacion" role="tabpanel" aria-labelledby="editar-tab">
              ...
              <form id="formulario_carga_cotizacion">
                <div class="modal-body">
                    ...
                    <div class="col-md-12">
                      <div class="form-group">
                        ...
                        <select class="form-control" id="edicion_proveedor" data-plugin="select2" style="width: 100%;">
                            ...
                            <?php
                            // Attempt select query execution
                            $sql_proveedores = "SELECT * FROM proveedores ORDER BY contacto";
                            mysql_query("SET NAMES 'utf8'");
                            if($result_proveedores = mysqli_query($conexion, $sql_proveedores)){
                                if(mysqli_num_rows($result_proveedores) > 0){
                                    $i = 0;
                 ...
                 <div id="cotizacion_pagos_avatar" class="col-md-12">
							<div class="cotizacion_pagos_container" data-id_registro="" data-id_proyecto="">
								<div class="d-flex justify-content-between">
									<h3>Editar Pagos</h3>
									<h3 class="text-right">Total: $<span id="total_cotizacion2" class="subtotal_categoria"></span></h3>
								</div>
    ...
    <script type="text/javascript">
      ...
      function funciones_cotizaciones(){
        ...
        $('.mensaje_cotizacion').click(function(){
          ...
          console.log("Proyecto: ",proyecto);
            ...
            $('#boton_mensaje').click(function(){             
              ...
              let proyecto = '<?php echo $_GET['id'];?>';
              let rubro_msg= $("#rubro").val();
              let categoria_msg= $("#categoria").val();
              let item_msg= $("#item").val();
              let condicion_msg= $("#condicion").val();
              let jornadas_msg= $("#jornadas").val();
              let cantidad_msg= $("#cantidad").val();
              let valor_msg= $("#valor").val();
              let total_msg= $("#total").val();
              let mensaje_msg=$("#motivo_cambio_estado").val();
          ...
          setTimeout(
            function (){
              $(".cotizacion_pagos_cargar_factura").each(function(){
                ...
                $(this).attr("data-id_proy_reg_pago", id_proy_reg_pago);
                ...
                /*if (proveedor == "" || proveedor == undefined){
                  ...
                  $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'json',
                    data: 'id_proy_reg_pago='+id_proy_reg_pago+'&registro='+ registro,
                    success:function(data){
                      console.log(data);
                      if (data == 1){
        ...
        $('.editar_proveedor_cotizacion').click(function () {
         // PARA EDITAR EL CONTENIDO DE CADA COTIZACION      
          let rubro = $(this).parents("tr").attr("data-rubro");
          let categoria = $(this).parents("tr").attr("data-categoria");
          let item = $(this).parents("tr").attr("data-item");
          let condicion = $(this).parents("tr").attr("data-condicion");
          let jornadas = $(this).parents("tr").attr("data-jornadas");
          let cantidad = $(this).parents("tr").attr("data-cantidad");
          let valor = $(this).parents("tr").attr("data-valor");
          let total = $(this).parents("tr").attr("data-total");
          let proveedor = $(this).parents("tr").attr("data-proveedor");
          let id_registro = $(this).parents("tr").attr("data-id_registro");
          ...
          $.ajax({
              ...
              success:function(response){
                  ...
                  $(".dropdown_proveedor_linea").empty();
                  for( var i = 0; i<len; i++){
                      var id = response[i]['id'];
                      var contacto = response[i]['contacto'];
                      if (id == proveedor){
                        $(".dropdown_proveedor_linea").append("<option value='"+id+"' selected>"+contacto+"</option>");
                      } else {
                        $(".dropdown_proveedor_linea").append("<option value='"+id+"'>"+contacto+"</option>");
                      }
          ...
          $(".dropdown_proveedor_linea").select2({
            tags: true
          });

        });

        $('.boton_guardar_cotizacion').on('click', function(){
        ...
        $('#boton_guardar_factura_cotizacion').click(function(){
          let id_pago = $('#boton_guardar_factura_cotizacion').attr('data-id_pago');
          let id_registro = $('#boton_guardar_factura_cotizacion').attr('data-id_registro');
          let id_proyecto = '<?php echo $_GET['id'];?>';
          let id_proveedor = $('#boton_guardar_factura_cotizacion').attr('data-id_proveedor');
          let forma_pago = $("#forma_pago_nuevo").val();
          let tipo_factura = $("#tipo_factura_nuevo_registro").val();
          let numero_factura = $("#numero_factura_nuevo_registro").val();
          let fecha_factura = $("#fecha_facturacion").val();
          let fecha_pactada = $("#fecha_pago").val();
          let importe_neto = $("#importe_neto").val();
          let iva = $("#iva").val();
          ...
          if (archivo_adjunto == ""){
            archivo_adjunto = "sin_subir";
            console.log("Archivo adjunto vacio, ",archivo_adjunto);
          } else {
            let adjunto = $(".upload_file")[0].files[0];
            archivo_adjunto = adjunto.name;
            console.log("Guardo en la DB el adjunto: ",archivo_adjunto);
          }
        ...
        $('.cargar_proveedor_cotizacion').click(function(){
          ...
          $.ajax({
              ...
              success:function(data){
                ...
                $('#edicion_proveedor').val(data.id_proveedor);
                $('#edicion_proveedor').trigger('change');
                $('#ot').val(data.id);
        ...
        $('#fecha_facturacion').change(function(){
          var fecha = new Date(document.getElementById('fecha_facturacion').value);
          var dias = document.getElementById('tiempo_pago').value;
          dias = parseInt(dias);
          fecha_calculo = fecha.setDate(fecha.getDate() + dias);
     	  console.log(fecha_calculo);
     	  document.getElementById("fecha_pago").valueAsDate = fecha;
        });
        ...
        importe_neto = $("#importe_neto").val();
          if ($('#tipo_factura_nuevo_registro').val() == '1' || $('#tipo_factura_nuevo_registro').val() == '2' || $('#tipo_factura_nuevo_registro').val() == '5') {
              ...
              $('#iva').attr('disabled', 'disabled').val('');
	            ...
	            $('#importe_bruto').attr('disabled', 'disabled').val(importe_neto);
	            $('#numero_factura_nuevo_registro').attr('disabled', 'disabled').val('');
	            $('#fecha_facturacion').attr('disabled', 'disabled');
          }
