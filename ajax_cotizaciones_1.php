<?php
  ...
  $merge_items = array();
    ...
    $sql_merge_categorias = "SELECT rubro_cotizacion AS rubro, categoria_cotizacion AS categoria, COUNT(*) AS merge
                            	FROM registros WHERE id_proyecto = '$proyecto' AND estado_registro <> 3
  ...
  function modal_cotizacion_pagos($registro, $proyecto) {
    ...
    <div class="modal fade cotizacion_pagos_interfaz" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      ...
      $sql_registros = "SELECT * FROM registros r
                        ...
                        WHERE id_proyecto = '$proyecto' AND estado_registro <> 3
      ...
      if($result_registros = mysqli_query($conexion, $sql_registros)){
          ...
          if ($row_registros['mensaje_cotizacion'] == 0){
            echo "<td scope='row' $attr_item class='triangulo_mensaje_off mensaje_items $class_item' data-item='".$row_registros['item']."' data-mensaje='".$row_registros['mensaje_cotizacion']."' data-registro='".$row_registros['id']."'>";
          } else {
            echo "<td scope='row' $attr_item class='triangulo_mensaje_on mensaje_items $class_item' data-item='".$row_registros['item']."' data-mensaje='".$row_registros['mensaje_cotizacion']."' data-registro='".$row_registros['id']."'>";
          }
